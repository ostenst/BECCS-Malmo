...
intervals = [(0.68, 0.86), (0.86, 0.90), (0.90, 0.94), (0.94, 0.98), (0.98, 1.02)]  
labels = ["0.68-0.86", "0.86-0.90", "0.90-0.94", "0.94-0.98", "0.98-1.02"] 
...
for start, end in intervals:
    data_interval = data_single[(data_single["cASU"] >= start) & (data_single["cASU"] < end)]
    
    box_data.append(data_interval["regret_2"])
    
    # Calculate density (fraction with regret_2 > 0)
    if len(data_interval) > 0:
        density = (data_interval["regret_2"] > 0).mean()
    else:
        density = 0
    densities.append(density)

print("Densities (fraction with regret_2 > 0):")
for i, (start, end) in enumerate(intervals):
    print(f"cASU {start}-{end}: {densities[i]:.3f}")
...
ax1.set_ylabel("Regret 2", fontsize=14)
ax1.set_title("Regret 2 Distribution by cASU Intervals", fontsize=16)
...
tick_labels = ["0.68", "0.86", "0.90", "0.94", "0.98", "1.02"]  # Start and end values of intervals
...
# # Calculate density (fraction with regret_2 > 0) for each bin
# scatter_densities = []
...
ax2.scatter(sampled_data['capex_ratio'], sampled_data['regret_2'], 
           alpha=0.6, s=20, color='gray', edgecolors='black', linewidth=0.5)

ax2.set_xlabel("Capex Ratio (Amine/Oxy)", fontsize=14)
ax2.set_title("Regret 2 vs Capex Ratio (Amine/Oxy)", fontsize=16)
...
# ax2_twin.plot(bin_centers, scatter_densities, 'r-', linewidth=2, marker='o', markersize=6, label='Density (regret_2 > 0)')
